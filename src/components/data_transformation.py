...
class DataTransformation:
    ...
    def get_data_tranformer_objects(self):
        ...
        try:
            ...
            sclar_step=('scaler',RobustScaler())

            preprocessor=Pipeline(
                steps=[imputer_step,sclar_step]
            )
    ...
    def initiate_data_transformer(self):
        ...
        try:
            ...
            dataframes=self.get_data(feature_store_file_path=self.feature_store_file_path)

            X=dataframes.drop(columns=TARGET_COLUMN)

            Y=np.where(dataframes[TARGET_COLUMN]==-1,0,1)

            X_train,X_test,y_train,y_test=train_test_split(X,Y,test_size=0.2)

            preprocessor=self.get_data_tranformer_objects()
